...
PATH_DWIE_COREF_WL_PREDICTIONS = "data/DWIE/COREF/SPACY"
SENTENCE_END_CHAR = [".", "?", "!"]
...
def rebuild_mention(start, end, value, mapped_doc) -> str:
    ...
    """
    rebuilt_mention = None
    for elem in mapped_doc:
        if elem["span"][0] <= start <= end <= elem["span"][1]:
            rebuilt_mention = elem["name"]
    return rebuilt_mention


def get_corefs(filename: str, mapped_doc: list) -> list:
    ...
    """
    with open(
        os.path.join(PATH_DWIE_COREF_WL_PREDICTIONS, filename.split(".json")[0]), "rb"
    ) as prediction_file:
        predicted_corefs = pickle.load(prediction_file)

    corefs = []
    for key, spans in predicted_corefs.items():
        coref = []
        for (value, start, end) in spans:
            rebuilt_mention = rebuild_mention(start, end, value, mapped_doc)
            if rebuilt_mention:
...
def pre_process_file(filename):
    ...
    corefs = get_corefs(filename, mapped_doc)
