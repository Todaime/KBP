...
PATH_DWIE_COREF_WL = "data/DWIE/COREF/WL/dwie_data.json"
PATH_DWIE_COREF_WL_PREDICTIONS = "data/DWIE/COREF/WL/"
PATH_DEFAULT_CONFIG_FILE = "src/models/COREF/WLCOREF/config.toml"
...
if __name__ == "__main__":
    ...
    argparser.add_argument("--config-file", default=PATH_DEFAULT_CONFIG_FILE)
    ...
    with jsonlines.open(PATH_DWIE_COREF_WL, mode="r") as input_data:
        input_data = [doc for i, doc in enumerate(input_data) if i != 419]
    ...
    for doc in docs:
        with open(
            os.path.join(PATH_DWIE_COREF_WL_PREDICTIONS, doc["document_id"][2:]),
            mode="wb",
        ) as outfile:
