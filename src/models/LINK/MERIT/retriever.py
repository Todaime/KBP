...
class DualEncoder(nn.Module):
    def __init__(self, mention_encoder, dot=False):
        super().__init__()
        self.loss_fct = MultiLabelLoss()
    ...
    def encode(
        ...
        if candidate_token_ids is not None:
            ...
            # B X C X L --> BC X L
            candidates_embeds = self.mention_encoder(
        ...
        if entity_token_ids is not None:
            ...
            entity_embeds = self.mention_encoder(
                input_ids=entity_token_ids, attention_mask=entity_masks
            )[0][:, 0, :]
        return mention_embeds[:, 0, :], candidates_embeds, entity_embeds
    ...
    def forward(
        ...
        if self.dot:
            logits = torch.dot(mention_embeds, candidates_embeds)
        else:
            logits = self.cos(mention_embeds, candidates_embeds)
